...
def check_path(path):
    ...
    check_list = []
    path_is_valid = os.path.lexists(path)
    if path_is_valid:
        check_list.append(os.path.isabs(path))
        check_list.append(os.path.isdir(path))
        check_list.append(os.path.isfile(path))
    return path_is_valid, check_list
...
def read_csv(file):
    ...
    with open(file, 'r') as f:
        for _ in f: # for line in f but we do not need to use the variable
            cnt += 1
    return cnt
